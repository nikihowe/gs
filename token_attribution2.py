    ...
    try:
        with torch.no_grad():
            ...
            if shift_logits.shape[1] == 0 or shift_labels.shape[1] == 0:
                ...
                print(
                    f'Shift logits shape: {shift_logits.shape}, Shift labels shape: {shift_labels.shape}'
                )
                return []
            if shift_logits.shape[1] != shift_labels.shape[1]:
                ...
                print(
                    f'Adjusted shapes to: {shift_logits.shape}, {shift_labels.shape}'
                )
    ...
    for i, token_index_in_sequence in enumerate(prompt_content_indices):
        ...
        try:
            with torch.no_grad():
                ...
                if shift_logits.shape[1] == 0 or shift_labels.shape[1] == 0:
                    print(
                        f'Error: Cannot calculate masked loss for token {i} due to zero-length logits/labels.'
                    )
                    ...
                    print(
                        f'Error: Mismatch masked logits/labels for token {i}. Adjusting.'
                    )
                    min_len = min(shift_logits.shape[1], shift_labels.shape[1])
                    shift_logits = shift_logits[:, :min_len, :]
                    shift_labels = shift_labels[:, :min_len]
...
if __name__ == '__main__':
    ...
    examples_text = [
        ...
        for text, label in zip(
            dataset.get('text', []), dataset.get('harmful', [])
        )
        if isinstance(label, (int, float))
        and int(label) == target_label  # Check type and value
    ]
    examples_prompts = [
    ...
    print(
        f'Found {num_available_examples} examples with label {target_label}.'
    )
    ...
    for i in range(num_examples_to_show):
        ...
        try:
            ...
            if not base_model_completion:
                print('Warning: Base model generated empty completion.')
        ...
        try:
            ...
            if not tuned_model_completion:
                print('Warning: Tuned model generated empty completion.')
        except Exception as e:
